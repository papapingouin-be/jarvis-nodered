const state = { services: [], current: null, editor: null, currentCodePath: null };
...
async function api(action, data = {}) {
  ...
  if (!response.ok) {
    const msg = json.error ? (json.error + (json.message ? ` — ${json.message}` : '')) : (`HTTP ${response.status}`);
    throw new Error(msg);
  }
  if (json.error) {
...
async function bootstrap(){
  try {
    ...
    document.getElementById('serviceMeta').textContent = `PHP ${health.php_version} · DB ${health.db_path} · writable ${health.db_dir_writable ? 'oui' : 'non'}`;
    await loadServices();
...
async function selectService(name){
  try {
    ...
    [...document.querySelectorAll('#serviceList button')].forEach(btn => btn.classList.toggle('active', btn.dataset.service === name));
    document.getElementById('serviceTitle').textContent = state.current.name;
    document.getElementById('serviceMeta').textContent = `${state.current.description || 'Sans description'} · moteur ${state.current.engine_default || 'plan_only'} · profil ${state.current.profile || 'default'}`;
    setGlobalStatus('');
    showConfig();
  } catch (e) {
    setGlobalStatus(e.message || String(e));
...
function ensureCurrent(){ if (!state.current) { setGlobalStatus('Aucun service sélectionné.'); return false; } return true; }
...
function showConfig(){
  ...
  let html = `<h3 style="margin-top:0">Configuration du service</h3><div class="small">Profil actif : <code>${esc(profile)}</code></div><label>Profil</label><input id="cfg_profile" value="${esc(profile)}" placeholder="default">`;
  if (!req.length) {
    html += `<div class="status ok">Ce service n'a pas déclaré de besoins de configuration.</div>`;
  } else {
    req.forEach(r => {
      const value = values[r.key] ?? '';
      html += `<label>${esc(r.label || r.key)} ${r.required ? ' *' : ''}</label><input id="cfg_${esc(r.key)}" value="${esc(value)}" placeholder="${esc(r.key)}"><div class="small">${esc(r.namespace || state.current.name)} / ${esc(r.key)}</div>`;
    });
  }
...
async function saveConfig(){
  ...
  try {
    ...
    const config = {};
    req.forEach(r => { const el = document.getElementById('cfg_' + r.key); config[r.key] = el ? el.value : ''; });
    await api('save_service_config', { service: state.current.name, profile, config });
    ...
    showConfig();
  } catch (e) { setGlobalStatus(e.message || String(e)); }
}
...
function buildSamplePayload(kind){
  ...
  if (kind === 'trace') return { ...sample, _meta: { trace: true, capture_logs: true, timeout: 30 } };
  return {};
}
...
function showTest(){
  ...
  document.getElementById('view').innerHTML = `
    ...
    <div class="row">
      <div><label>Moteur</label><select id="test_engine"><option value="plan_only"${engine === 'plan_only' ? ' selected' : ''}>plan_only</option><option value="python_direct"${engine === 'python_direct' ? ' selected' : ''}>python_direct</option></select></div>
      <div><label>Timeout (s)</label><input id="test_timeout" type="number" value="30" min="1" max="300"></div>
      <div><label>Profil de config</label><input id="test_profile" value="${esc(state.current.profile || 'default')}"></div>
    </div>
    <label>Payload JSON</label><textarea id="jsonInput">{}</textarea>
    ...
    <h4>Résultat</h4><pre id="testResult">Aucune exécution.</pre>`;
  fillPayload('sample');
}
...
async function runTest(){
  ...
  try {
    ...
    let payload;
    try { payload = JSON.parse(raw); } catch { throw new Error('Le JSON de test est invalide.'); }
    document.getElementById('testResult').textContent = 'Exécution en cours...';
    const result = await api('run_service_test', { service: state.current.name, engine, timeout, profile, payload });
...
async function showCode(){
  ...
  try {
    ...
    document.getElementById('view').innerHTML = `
      ...
      <div class="row"><div><label>Fichier</label><select id="code_file">${fileOptions}</select></div></div>
      <div class="toolbar" style="margin-top:12px"><button onclick="loadCodeFile()">Charger</button><button class="secondary" onclick="saveCodeFile()">Sauver</button><button class="secondary" onclick="validateCodeFile()">Valider</button></div>
      <div id="editor"></div><h4>Diagnostic</h4><pre id="codeDiag">Aucun diagnostic.</pre>`;
    if (!window.require) {
...
async function showRuns(){
  ...
  try {
    ...
    if (!runs.length) {
      ...
      html += `<table><thead><tr><th>ID</th><th>Date</th><th>Service</th><th>Moteur</th><th>Statut</th><th>Résumé</th></tr></thead><tbody>`;
      runs.forEach(run => {
        html += `<tr><td>${esc(run.run_id)}</td><td>${esc(run.started_at)}</td><td>${esc(run.service_name)}</td><td>${esc(run.engine)}</td><td>${esc(run.status)}</td><td>${esc(run.summary || '')}</td></tr>`;
      });
      html += `</tbody></table>`;
    }
    document.getElementById('view').innerHTML = html;
