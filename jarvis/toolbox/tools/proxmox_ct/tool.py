...
SERVICE_DEFINITIONS: dict[str, dict[str, Any]] = {
    ...
    "list-ct": {
        "phase": "collect",
        "confirmed_required": False,
        "description": "List Proxmox containers (pct list).",
        "required_params": [],
        "optional_params": ["ssh_target"],
    },
    "list-templates": {
        "phase": "collect",
        "confirmed_required": False,
        "description": "List Proxmox templates (pveam available --section system).",
        "required_params": [],
        "optional_params": ["ssh_target"],
    },
    "preflight-create": {
...
def _mode_doc() -> dict[str, Any]:
    return {
        ...
        "modes": {
            ...
            "list-ct": "liste les conteneurs Proxmox (pct list)",
            "list-templates": "liste les templates disponibles (pveam available --section system)",
...
def _run_mode(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if mode == "list-ct":
        out = _run_cmd(["bash", "-lc", "pct list"], ssh_target=ssh_target)
        return {"mode": mode, "ssh_target": ssh_target, "containers": out}

    if mode == "list-templates":
        out = _run_cmd(
            ["bash", "-lc", "pveam available --section system | sed -n '1,120p'"],
            ssh_target=ssh_target,
        )
        return {"mode": mode, "ssh_target": ssh_target, "templates": out}
